...
A modern expandable/expandable card component for React. It displays a set of cards, and clicking a card expands it into a modal with more details — complete with smooth motion transitions and blur overlay.
...
You can install the ExpandableCard component using the recommended CLI, or manually copy the implementation:

<InstallToggle
  cliCode="npx shadcn@latest add http://localhost:3000/r/expendable-card.json"
  manualCode={`
  'use client';
...
To use the ExpandableCard component, import it in your app:

<CodeBlock code={`import ExpendableCard from '@/components/ExpendableCard';`} />
...
To customize cards and content, manually edit the array and markup inside the component file.

<hr style={{ borderColor: "var(--color-neutral-900)", borderWidth: 1 }} />

## Features

- **Animated expand/collapse:** Card smoothly expands into a modal with transitions.
- **Backdrop/blur:** Overlay with blur and click-to-dismiss.
- **Motion-powered:** Uses [motion.dev](https://motion.dev/) animation for smooth layout transitions.
- **No props — manual customization:** Customize the card set/content directly in the file.

<hr style={{ borderColor: "var(--color-neutral-900)", borderWidth: 1 }} />

## Customization

- Edit the `cards` array in the component file to change cards, images, and descriptions.
- Change appearance, animation timing, and layout in the JSX as needed.
- Use Tailwind CSS classes via `className` for coloring, sizing, and layout tweaks.

<hr style={{ borderColor: "var(--color-neutral-900)", borderWidth: 1 }} />

## Troubleshooting

- If animation does not work, check that you're using [motion.dev](https://motion.dev/).
- If your cards or content need to be changed, edit the card array inside the component.
- Make sure your images are allowed by your Next.js image configuration if using remote URLs.
